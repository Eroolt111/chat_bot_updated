...
# -----------------------------------------------------------------------------
# BAC_BALANCE_DETAIL BLOCK - Bank's Internal Account Balances (дотоодын данс)
# -----------------------------------------------------------------------------
BAC_BALANCE_BLOCK = """
**━━━ DBM.BAC_BALANCE_DETAIL RULES (Дотоодын дансны үлдэгдэл - Internal Account Balances) ━━━**

 **PURPOSE:** Daily snapshots of the bank's internal account balances.
   This contains balance information for the bank's own internal accounts.

   **KEYWORDS → Use this table when user says:**
   - дотоодын данс, дотоод данс, банкны данс
   - дансны үлдэгдэл (when referring to internal accounts, not customer loans)
   - BAC данс

 **⚠️ DISTINGUISH FROM OTHER TABLES:**
   - BAC_BALANCE (this) = Bank's internal account balances
   - LOAN_BALANCE = Customer loan accounts (зээлдэгчийн данс)
   - PLACEMENT/BORROWING = Interbank transactions

 **KEY COLUMNS:**
   - `txn_date`: Snapshot date (for latest state: MAX(txn_date))
   - `acnt_code`: Unique internal account identifier
   - `acnt_name`: Account name/description
   - `status`: O=Open, C=Closed, N=New, D=Deleted
   - `prod_type`: Product type (ASSET, LIABILITY, EQUITY, INCOME, EXPENSE)
   - `prod_code`, `prod_name`: Product code and name
   - `cur_code`, `rate`: Currency and exchange rate
   - `crnt`: Current balance (CRITICAL - this is the balance column!)
   - `start_date`, `end_date`: Account period dates
   - `opened_date`, `closed_date`: Account lifecycle dates
   - `rating`, `rating_name`: Rating score and name

 **DEDUPLICATION:** Daily snapshots - use global snapshot pattern:
   ```sql
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.BAC_BALANCE_DETAIL)
   ```

 **PATTERNS:**

   -- All active internal accounts with balances
   SELECT acnt_code, acnt_name, crnt, cur_code, prod_type, prod_name, status
   FROM DBM.BAC_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.BAC_BALANCE_DETAIL)
     AND status = 'O'
   ORDER BY (crnt * rate) DESC

   -- Internal accounts by product type
   SELECT acnt_code, acnt_name, crnt, cur_code, prod_type, prod_name
   FROM DBM.BAC_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.BAC_BALANCE_DETAIL)
   ...
   -- Total balances by product type and currency
   SELECT prod_type, cur_code, SUM(crnt) AS total_balance, COUNT(*) AS acnt_count
   FROM DBM.BAC_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.BAC_BALANCE_DETAIL)
     AND status = 'O'
   GROUP BY prod_type, cur_code
   ORDER BY prod_type, SUM(crnt * rate) DESC

   -- Search by account name
   SELECT acnt_code, acnt_name, crnt, cur_code, prod_type, rating_name
   FROM DBM.BAC_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.BAC_BALANCE_DETAIL)
     AND UPPER(acnt_name) LIKE UPPER('%{{SEARCH_TERM}}%')
     AND status = 'O'
   ORDER BY (crnt * rate) DESC
"""

# -----------------------------------------------------------------------------
# CASA_BALANCE BLOCK - Customer Account Balances (харилцагчийн данс)
# -----------------------------------------------------------------------------
CASA_BALANCE_BLOCK = """
**━━━ DBM.CASA_BALANCE RULES (Харилцагчийн дансны үлдэгдэл - Customer Account Balances) ━━━**

 **PURPOSE:** Daily snapshots of customer account balances.
   This contains customer accounts (харилцах данс) used for deposits and transactions.

   **KEYWORDS → Use this table when user says:**
   - харилцагчийн данс, харилцах данс, CASA данс

 **⚠️ DISTINGUISH FROM OTHER TABLES:**
   - CASA_BALANCE (this) = Customer deposit/current accounts (харилцагчийн данс)
   - LOAN_BALANCE = Customer LOAN accounts (зээлийн данс) - different!
   - BAC_BALANCE = Bank's internal accounts (дотоодын данс)

 **KEY COLUMNS:**
   - `txn_date`: Snapshot date (for latest state: MAX(txn_date))
   - `acnt_code`: Unique customer account identifier
   - `acnt_name`: Account name/description
   - `customer_code`, `customer_name`: Customer identifier and name
   - `prod_code`, `prod_name`: Product code and name
   - `acnt_type`: Account type (CA=Харилцах/Current Account)
   - `acnt_manager`, `acnt_manager_name`: Account manager ID and name
   - `status`: O=Open, C=Closed, N=New, D=Deleted
   - `cur_code`: Currency code
   - `current_bal`: Current balance (CRITICAL - this is the balance column!)
   - `open_date`, `closed_date`: Account lifecycle dates

 **DEDUPLICATION:** Daily snapshots - use global snapshot pattern:
   ```sql
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.CASA_BALANCE)
   ```

 **PATTERNS:**

   -- All active customer accounts with balances
   SELECT acnt_code, acnt_name, customer_name, current_bal, cur_code, prod_name, status
   FROM DBM.CASA_BALANCE
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.CASA_BALANCE)
     AND status = 'O'
   ORDER BY (current_bal * rate) DESC

   -- Customer's accounts by customer name
   SELECT acnt_code, acnt_name, customer_name, current_bal, cur_code, prod_name, acnt_manager_name
   FROM DBM.CASA_BALANCE
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.CASA_BALANCE)
     AND UPPER(customer_name) LIKE UPPER('%{{CUSTOMER_NAME}}%')
     AND status = 'O'
   ...
   -- Accounts by manager
   SELECT acnt_code, acnt_name, customer_name, current_bal, cur_code, acnt_manager_name
   FROM DBM.CASA_BALANCE
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.CASA_BALANCE)
     AND UPPER(acnt_manager_name) LIKE UPPER('%{{MANAGER_NAME}}%')
     AND status = 'O'
   ORDER BY (current_bal * rate) DESC

   -- Total balances by currency
   SELECT cur_code, SUM(current_bal) AS total_balance, COUNT(*) AS acnt_count
   FROM DBM.CASA_BALANCE
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.CASA_BALANCE)
     AND status = 'O'
   GROUP BY cur_code
   ORDER BY SUM(current_bal * rate) DESC

   -- Highest balance customer accounts
   SELECT acnt_code, acnt_name, customer_name, current_bal, cur_code, prod_name
   FROM DBM.CASA_BALANCE
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.CASA_BALANCE)
     AND status = 'O' AND current_bal > 0
   ORDER BY (current_bal * rate) DESC
   FETCH FIRST 10 ROWS ONLY
"""

# -----------------------------------------------------------------------------
# FX_DEAL_DETAIL BLOCK - Foreign Exchange Deals (валютын хэлцэл)
# -----------------------------------------------------------------------------
FX_DEAL_BLOCK = """
**━━━ DBM.FX_DEAL_DETAIL RULES (Валютын хэлцэл - FX Trading Deals) ━━━**

 **PURPOSE:** FX trading deals executed by the bank (SPOT, FORWARD, SWAP).
   Each row represents one deal or one leg of a deal's definition.
   Use this to understand deal properties, counterparties, and deal terms.

   **KEYWORDS → Use this table when user says:**
   - валютын арилжаа, валютын хэлцэл, FX хэлцэл
   - спот арилжаа, форвард арилжаа, своп арилжаа
   - худалдаж авсан валют, зарсан валют
   - диллер, арилжаа хийсэн

 **⚠️ DISTINGUISH FROM OTHER TABLES:**
   - FX_DEAL_DETAIL (this) = Deal definitions (one row per deal/leg)
   - FX_SWAP_BALANCE_DETAIL = Daily balances for SWAP deals only (multiple rows over time)
   - **IMPORTANT:** Only IS_SWAP=1 deals have entries in FX_SWAP_BALANCE_DETAIL

 **KEY COLUMNS:**
   - `deal_code`: Unique deal identifier (primary key)
   - `is_swap`: 0=Not swap, 1=Swap deal. **CRITICAL for filtering**
   - `customer_name`: Counterparty name
   - `prod_type`: SPOT, FORWARD
   - `buy_sell`: B=Buy (Худалдаж авсан), S=Sell (Зарсан)
   - `deal_status`: N=New, S=Started, SS=Settlement Started, SF=Settlement Finished, C=Closed, D=Cancelled
   - `buy_amount`, `buy_cur_code`: Amount and currency being bought
   - `sell_amount`, `sell_cur_code`: Amount and currency being sold
   - `deal_rate`: Exchange rate for the deal
   - `profit_loss`: Profit/loss from deal
   - `leg_number`: For swaps: 1=near leg, 2=far leg
   - `dealer_name`: Trader who executed the deal

 **SWAP DEAL HANDLING:**
   - Swap deals (IS_SWAP=1) may have 2 rows: leg_number=1 (near), leg_number=2 (far)
   - To get swap daily balances/interest → use FX_SWAP_BALANCE_DETAIL
   - Non-swap deals do NOT have daily balances

 **PATTERNS:**

   -- All active FX deals
   SELECT deal_code, customer_name, prod_type, buy_sell, buy_amount, buy_cur_code,
          sell_amount, sell_cur_code, deal_rate, deal_status, deal_date
   FROM DBM.FX_DEAL_DETAIL
   WHERE deal_status NOT IN ('C', 'D')
   ORDER BY deal_date DESC

   -- SPOT deals only
   SELECT deal_code, customer_name, buy_sell, buy_amount, buy_cur_code,
          sell_amount, sell_cur_code, deal_rate, profit_loss, dealer_name
   FROM DBM.FX_DEAL_DETAIL
   WHERE prod_type = 'SPOT' AND deal_status NOT IN ('C', 'D')
   ORDER BY deal_date DESC

   -- SWAP deals only (near leg)
   SELECT deal_code, swap_code, customer_name, buy_sell, buy_amount, buy_cur_code,
          sell_amount, sell_cur_code, deal_rate, swap_give_int, swap_take_int
   FROM DBM.FX_DEAL_DETAIL
   WHERE is_swap = 1 AND leg_number = 1 AND deal_status NOT IN ('C', 'D')
   ORDER BY deal_date DESC

   -- Deals by counterparty
   SELECT deal_code, prod_type, buy_sell, buy_amount, buy_cur_code,
          sell_amount, sell_cur_code, deal_rate, deal_date, deal_status
   FROM DBM.FX_DEAL_DETAIL
   WHERE UPPER(customer_name) LIKE UPPER('%{{CUSTOMER_NAME}}%')
   ORDER BY deal_date DESC

   -- Total profit/loss by product type
   SELECT prod_type, SUM(profit_loss) AS total_profit_loss, COUNT(*) AS deal_count
   FROM DBM.FX_DEAL_DETAIL
   WHERE deal_status NOT IN ('D')
   GROUP BY prod_type
   ORDER BY SUM(profit_loss) DESC
"""

# -----------------------------------------------------------------------------
# FX_SWAP_BALANCE_DETAIL BLOCK - Swap Daily Balances (swap үлдэгдэл)
# -----------------------------------------------------------------------------
FX_SWAP_BALANCE_BLOCK = """
**━━━ DBM.FX_SWAP_BALANCE_DETAIL RULES (Swap хэлцлийн үлдэгдэл - Swap Daily Balances) ━━━**

 **PURPOSE:** Daily financial snapshots for SWAP deals ONLY.
   Each row represents one swap leg on one specific TXN_DATE.
   Use this to track how swap deals evolve over time (balances, interest, revaluation).

   **KEYWORDS → Use this table when user says:**
   - swap үлдэгдэл, своп үлдэгдэл
   - swap хүүний хуримтлал, своп хүү
   - swap авлага, swap өглөг
   - үнэлгээний тэгшитгэл (revaluation)
   - тэнцлийн гадуурх (off-balance)

 **⚠️ CRITICAL RULES:**
   - This table contains ONLY swap deals (IS_SWAP=1 from FX_DEAL_DETAIL)
   - Non-swap deals do NOT have entries here
   - Use FX_DEAL_DETAIL to understand what a deal is
   - Use this table to understand how a swap deal evolves over time

 **KEY COLUMNS:**
   - `txn_date`: Snapshot date (use MAX(txn_date) for latest state)
   - `deal_code`, `swap_code`: Deal and swap identifiers
   - `customer_name`: Counterparty name
   - `deal_status`: N=New, S=Started, SS=Settlement Started, SF=Finished, C=Closed, D=Cancelled
   - `buy_amount_bal`: Receivable balance | `sell_amount_bal`: Payable balance
   - `int_rcv`: Accrued interest receivable | `int_pay`: Accrued interest payable
   - `reval_inc`: Revaluation income | `reval_exp`: Revaluation expense
   - `profit_loss`: Current profit/loss
   - `days_to_maturity`: Days until deal maturity
   - `leg_number`: 1=near leg, 2=far leg
   - `swap_give_int_rate`: Swap Give Interest Rate.
   - `swap_take_int_rate`: Swap Take Interest Rate.
   - 'buy_amount_bal': The outstanding receivable balance on the buy side.,
   - 'sell_amount_bal': The outstanding payable balance on the sell side.,
   - 'give_int_bal': The outstanding interest to be paid (Give Interest Balance).
   - 'take_int_bal': The outstanding interest to be received (Take Interest Balance).
   - 'int_rcv': Accrued interest receivable. 
   - 'int_pay': Accrued interest payable.
   - 'int_rcv_bill': Billed interest receivable.
   - 'int_pay_bill': Billed interest payable.

 **DEDUPLICATION:** Daily snapshots - use global snapshot pattern:
   ```sql
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.FX_SWAP_BALANCE_DETAIL)
   ```

 **PATTERNS:**

   -- All active swap balances (latest state)
   SELECT *
   FROM DBM.FX_SWAP_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.FX_SWAP_BALANCE_DETAIL)
     AND deal_status NOT IN ('C', 'D')
   ORDER BY days_to_maturity ASC

   -- Swap interest accruals
   SELECT deal_code, customer_name, int_rcv, int_pay,
          swap_give_int_rate, swap_take_int_rate, deal_status
   FROM DBM.FX_SWAP_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.FX_SWAP_BALANCE_DETAIL)
     AND (int_rcv > 0 OR int_pay > 0)
   ORDER BY int_rcv DESC

   -- Swap revaluation summary
   SELECT deal_code, customer_name, reval_asset, reval_liability,
          reval_inc, reval_exp, last_reval_date
   FROM DBM.FX_SWAP_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.FX_SWAP_BALANCE_DETAIL)
     AND deal_status NOT IN ('C', 'D')
   ORDER BY reval_inc DESC

   -- Off-balance sheet positions
   SELECT deal_code, customer_name, buy_amount_off, sell_amount_off,
          buy_cur_code, sell_cur_code
   FROM DBM.FX_SWAP_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.FX_SWAP_BALANCE_DETAIL)
     AND (buy_amount_off > 0 OR sell_amount_off > 0)
   ORDER BY buy_amount_off DESC

   -- Swaps maturing soon (next 30 days)
   SELECT deal_code, customer_name, days_to_maturity, buy_amount_bal, sell_amount_bal,
          profit_loss, deal_status
   FROM DBM.FX_SWAP_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.FX_SWAP_BALANCE_DETAIL)
     AND days_to_maturity <= 30 AND days_to_maturity > 0
     AND deal_status NOT IN ('C', 'D')
   ORDER BY days_to_maturity ASC

   -- Total swap profit/loss
   SELECT SUM(profit_loss) AS total_profit_loss, COUNT(DISTINCT deal_code) AS deal_count
   FROM DBM.FX_SWAP_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.FX_SWAP_BALANCE_DETAIL)
     AND deal_status NOT IN ('C', 'D')
"""
...
# -----------------------------------------------------------------------------
# COLL_BALANCE_DETAIL BLOCK - Collateral Balances (барьцаа хөрөнгө)
# -----------------------------------------------------------------------------
COLL_BALANCE_BLOCK = """
**━━━ DBM.COLL_BALANCE_DETAIL RULES (Барьцаа хөрөнгийн үлдэгдэл - Collateral Balances) ━━━**

 **PURPOSE:** Daily snapshots of collateral balances and linkage mapping.
   Shows what collateral exists, utilization, valuations, and what loans it secures.

   **KEYWORDS → Use this table when user says:**
   - барьцаа хөрөнгө, барьцаа, collateral
   - барьцааны үнэлгээ, зах зээлийн үнэлгээ
   - барьцаа хөрөнгийн багц, багц барьцаа
   - ашигласан барьцаа, боломжит барьцаа

 **⚠️ DISTINGUISH FROM OTHER TABLES:**
   - COLL_BALANCE (this) = Collateral assets pledged to secure loans
   - LOAN_BALANCE = Loan accounts (зээлийн данс)
   - SECURITY_BALANCE = Securities/bonds traded by bank

 **KEY COLUMNS:**
   - `txn_date`: Snapshot date (use MAX(txn_date) for latest state)
   - `acnt_code`: Unique collateral account identifier
   - `name`: Collateral description
   - `cust_code`, `cust_name`: Customer who pledged collateral
   - `coll_type`: 1=Хөдлөх, 2=Үл хөдлөх, 3=Үнэт цаас, 4=Хадгаламж, 5=Батлан даалт, 6=Бусад, 7=Багц, 8=Алт
   - `prod_type`: 'COLL'=Individual, 'COLL_POOL'=Pooled collateral
   - `cur_code`: Currency
   - `limit_amt`: Maximum credit limit from this collateral
   - `available`: Remaining available amount
   - `utilized`: Amount currently used to secure loans
   - `market_value`, `market_value_current`: Initial and current valuations
   - `status`: N=New, O=Open, C=Closed, D=Deleted
   - `status_add`: FR=Free, EN=Encumbered, BL=Blocked, IA=Inactive
   - `loan_acnts`: Comma-separated loan account codes secured by this collateral
   - `loan_count`: Number of loans secured
   - `commitment_acnts`, `commitment_count`: Commitment loans secured

 **DEDUPLICATION:** Daily snapshots - use global snapshot pattern:
   ```sql
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
   ```

 **PATTERNS:**

   -- All active collateral (latest state)
   SELECT acnt_code, name, cust_name, coll_type_name, limit_amt, available, utilized,
          market_value_current, cur_code, status, status_add_name
   FROM DBM.COLL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
     AND status = 'O'
   ORDER BY limit_amt DESC

   -- Collateral by type
   SELECT coll_type_name, cur_code, SUM(limit_amt) AS total_limit,
          SUM(utilized) AS total_utilized, SUM(available) AS total_available, COUNT(*) AS cnt
   FROM DBM.COLL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
     AND status = 'O'
   GROUP BY coll_type_name, cur_code
   ORDER BY SUM(limit_amt) DESC

   -- Real estate collateral (coll_type = 2)
   SELECT acnt_code, name, cust_name, sub_type_name, addr_detail,
          market_value_current, limit_amt, available, utilized, cur_code
   FROM DBM.COLL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
     AND coll_type = 2 AND status = 'O'
   ORDER BY market_value_current DESC

   -- Collateral pools
   SELECT acnt_code, name, cust_name, limit_amt, available, utilized,
          loan_count, commitment_count, cur_code
   FROM DBM.COLL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
     AND prod_type = 'COLL_POOL' AND status = 'O'
   ORDER BY limit_amt DESC

   -- Collateral by customer
   SELECT acnt_code, name, coll_type_name, limit_amt, available, utilized,
          market_value_current, cur_code, loan_count
   FROM DBM.COLL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
     AND UPPER(cust_name) LIKE UPPER('%{{CUSTOMER_NAME}}%')
     AND status = 'O'
   ORDER BY limit_amt DESC

   -- Fully utilized collateral (available = 0)
   SELECT acnt_code, name, cust_name, limit_amt, utilized, cur_code, loan_acnts
   FROM DBM.COLL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
     AND status = 'O' AND available = 0 AND utilized > 0
   ORDER BY utilized DESC

   -- Collateral with outdated valuations (older than 1 year)
   SELECT acnt_code, name, cust_name, market_value, market_value_date,
          market_value_current, market_value_current_date, cur_code
   FROM DBM.COLL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.COLL_BALANCE_DETAIL)
     AND status = 'O'
     AND (market_value_current_date IS NULL
          OR market_value_current_date < ADD_MONTHS(SYSDATE, -12))
   ORDER BY market_value DESC
"""

# -----------------------------------------------------------------------------
# GL_BALANCE_DETAIL BLOCK - General Ledger Balances (ерөнхий дэвтэр)
# -----------------------------------------------------------------------------
GL_BALANCE_BLOCK = """
**━━━ DBM.GL_BALANCE_DETAIL RULES (Ерөнхий дэвтрийн үлдэгдэл - General Ledger Balances) ━━━**

 **PURPOSE:** Daily snapshots of General Ledger (GL) account balances.
   Contains core accounting data including GL account balances, daily credits/debits, and audit information.

   **KEYWORDS → Use this table when user says:**
   - ерөнхий дэвтэр, GL данс, GL үлдэгдэл
   - дансны бүртгэл, нягтлан бодох бүртгэл
   - credit, debit, дебит, кредит
   - аудит, аудитын тэгшитгэл

 **⚠️ DISTINGUISH FROM OTHER TABLES:**
   - GL_BALANCE (this) = General Ledger/accounting data (нягтлан бодох бүртгэл)
   - BAC_BALANCE = Bank's internal accounts (дотоодын данс)
   - LOAN_BALANCE = Customer loan accounts (зээлийн данс)

 **KEY COLUMNS:**
   - `txn_date`: Snapshot date (use MAX(txn_date) for latest state)
   - `gl_acnt_code`: GL account code (unique identifier)
   - `gl_acnt_name`: GL account name
   - `gl_code`: GL classification code (1000, 2000, etc.)
   - `gl_code_name`: GL classification name (ХӨРӨНГӨ, ӨР ТӨЛБӨР, ӨӨРИЙН ХӨРӨНГӨ, ОРЛОГО, ЗАРДАЛ)
   - `cur_code`, `rate`: Currency and exchange rate
   - `balance`: Account balance in original currency
   - `balance_ba`: Account balance in MNT (base currency) - **use for cross-currency aggregation**
   - `day_ct`: Daily credit (орлого) | `day_dt`: Daily debit (зарлага)
   - `status`: **0=Inactive, 1=Active** (DIFFERENT from other tables which use O/C!)
   - `is_audit_entry`: 0=No audit, 1=Audit entry
   - `audit_net`, `audit_net_ba`: Audit adjustment amounts

 **⚠️ STATUS COLUMN WARNING:**
   Unlike other tables (LOAN, PLACEMENT, etc.) which use 'O'/'C':
   - GL_BALANCE uses NUMERIC status: **0=Inactive, 1=Active**
   - For active accounts: `AND status = 1`


 **DEDUPLICATION:** Daily snapshots - use global snapshot pattern:
   ```sql
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.GL_BALANCE_DETAIL)
   ```

 **PATTERNS:**

   -- All active GL accounts (latest state)
   SELECT gl_acnt_code, gl_acnt_name, gl_code_name, balance, balance_ba, cur_code, status
   FROM DBM.GL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.GL_BALANCE_DETAIL)
     AND status = 1
   ORDER BY balance_ba DESC

   -- GL accounts by classification
   SELECT gl_acnt_code, gl_acnt_name, balance, balance_ba, cur_code
   FROM DBM.GL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.GL_BALANCE_DETAIL)
     AND gl_code = '{{GL_CODE}}'
     AND status = 1
   ...
   -- Total balances by GL classification
   SELECT gl_code, gl_code_name, cur_code, SUM(balance) AS total_balance,
          SUM(balance_ba) AS total_balance_mnt, COUNT(*) AS acnt_count
   FROM DBM.GL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.GL_BALANCE_DETAIL)
     AND status = 1
   GROUP BY gl_code, gl_code_name, cur_code
   ORDER BY gl_code, SUM(balance_ba) DESC

   -- Daily credits and debits
   SELECT gl_acnt_code, gl_acnt_name, day_ct, day_dt, cur_code
   FROM DBM.GL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.GL_BALANCE_DETAIL)
     AND (day_ct > 0 OR day_dt > 0)
     AND status = 1
   ORDER BY (day_ct + day_dt) DESC

   -- Audit entries
   SELECT gl_acnt_code, gl_acnt_name, audit_net, audit_net_ba, cur_code
   FROM DBM.GL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.GL_BALANCE_DETAIL)
     AND is_audit_entry = 1
   ORDER BY audit_net_ba DESC

   -- Search by account name
   SELECT gl_acnt_code, gl_acnt_name, gl_code_name, balance, balance_ba, cur_code
   FROM DBM.GL_BALANCE_DETAIL
   WHERE txn_date = (SELECT MAX(txn_date) FROM DBM.GL_BALANCE_DETAIL)
     AND UPPER(gl_acnt_name) LIKE UPPER('%{{SEARCH_TERM}}%')
     AND status = 1
   ORDER BY balance_ba DESC
"""
...
DOMAIN_BLOCKS = {
    ...
    "dbm.bac_balance_detail": BAC_BALANCE_BLOCK,
    "dbm.casa_balance": CASA_BALANCE_BLOCK,
    "dbm.fx_deal_detail": FX_DEAL_BLOCK,
    "dbm.fx_swap_balance_detail": FX_SWAP_BALANCE_BLOCK,
    "dbm.coll_balance_detail": COLL_BALANCE_BLOCK,
    "dbm.gl_balance_detail": GL_BALANCE_BLOCK,
}
...
| **Internal Accounts** | дотоодын данс, дотоод данс, банкны данс, BAC данс | `dbm.bac_balance_detail` |
| **Customer Accounts** | харилцагчийн данс, харилцах данс, CASA | `dbm.casa_balance` |
| **FX Deals** | валютын арилжаа, валютын хэлцэл, спот, форвард, своп, диллер | `dbm.fx_deal_detail` |
| **Swap Balances** | swap үлдэгдэл, своп хүү, swap авлага/өглөг, үнэлгээний тэгшитгэл | `dbm.fx_swap_balance_detail` |
| **FX Deal + Swap Balance** | своп хэлцлийн дэлгэрэнгүй мэдээлэл | `dbm.fx_deal_detail` + `dbm.fx_swap_balance_detail` |
| **Collateral** | барьцаа хөрөнгө, барьцаа, барьцааны үнэлгээ, зах зээлийн үнэлгээ, барьцаа багц | `dbm.coll_balance_detail` |
| **General Ledger** | ерөнхий дэвтэр, GL данс, GL үлдэгдэл, нягтлан бодох бүртгэл, credit, debit, аудит | `dbm.gl_balance_detail` |
...
- `dbm.bac_balance_detail`: ДОТООДЫН ДАНС - bank's internal account balances (crnt = current balance)
- `dbm.casa_balance`: ХАРИЛЦАГЧИЙН ДАНС - customer deposit/current accounts (current_bal = balance)
- `dbm.fx_deal_detail`: ВАЛЮТЫН АРИЛЖААНЫ ХЭЛЦЭЛ - FX trading deals (SPOT, FORWARD, SWAP definitions)
- `dbm.fx_swap_balance_detail`: SWAP ХЭЛЦЭЛИЙН ҮЛДЭГДЭЛ - daily balances for SWAP deals only (use with fx_deal_detail)
- `dbm.coll_balance_detail`: БАРЬЦАА ХӨРӨНГӨ - collateral balances, valuations, and loan linkages
- `dbm.gl_balance_detail`: ЕРӨНХИЙ ДЭВТЭР - General Ledger account balances (status: 0=Inactive, 1=Active - different from other tables!)
...
- `dbm.bac_balance_detail` has daily snapshots - use same pattern
- `dbm.casa_balance` has daily snapshots - use same pattern
- `dbm.fx_swap_balance_detail` has daily snapshots - use same pattern
- `dbm.coll_balance_detail` has daily snapshots - use same pattern
- `dbm.gl_balance_detail` has daily snapshots - use same pattern

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
**NES SYSTEM MODULE ARCHITECTURE:**
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

The database follows Development Bank's NES banking system modules. **When a question relates to a module, consider ALL tables in that module:**

| Module | Mongolian Name | Tables | Join Keys | Usage |
|--------|---------------|--------|-----------|-------|
| LOAN | Зээлийн модуль | `loan_balance_detail`, `bcom_nrs_detail`, `loan_txn` | `acnt_code` | balance=current state, nrs=payment schedule, txn=actual transactions that made |
| FX | Валютын арилжааны модуль | `fx_deal_detail`, `fx_swap_balance_detail` | `deal_no` | deal=trade definition, swap_balance=daily valuations of swap deals |
| COLL | Барьцаа хөрөнгийн модуль | `coll_balance_detail` | `acnt_code` (links to LOAN) | Collateral securing loans |
| TREASURY | Эх үүсвэрийн модуль | `placement_balance_detail`, `borrowing_balance_detail` | — | placement=bank places money OUT, borrowing=bank borrows IN |
| SECURITIES | Үнэт цаасны модуль | `security_balance_detail` | — | Bonds, debt instruments |
| ACCOUNTS | Дансны модуль | `bac_balance_detail`, `casa_balance` | — | bac=bank internal accounts, casa=customer accounts |
| GL | Ерөнхий дэвтэр | `gl_balance_detail` | — | Accounting ledger |

**CROSS-MODULE RELATIONSHIPS (when to join across modules):**
- **LOAN ↔ COLL:** A loan's collateral → join `loan_balance_detail` + `coll_balance_detail` ON `acnt_code`
- **FX deal ↔ FX swap:** Swap deal definition + daily balances → join `fx_deal_detail` + `fx_swap_balance_detail` ON `deal_no`

**MODULE DETECTION — if these keywords appear, select ALL tables in that module:**
- "FX", "валют", "арилжаа", "своп" → FX module (both `fx_deal_detail` + `fx_swap_balance_detail`)
- "барьцаа", "барьцаа хөрөнгө" → COLL module
- "зээл" + ("хуваарь" or "төлбөр" or "гүйлгээ") → LOAN module (multiple tables needed)
- "эх үүсвэр", "татсан болон байршуулсан" → TREASURY module (both tables)
- "данс" without "дотоод"/"харилцах" → may need both ACCOUNTS tables
