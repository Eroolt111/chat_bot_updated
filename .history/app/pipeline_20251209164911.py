    ...
    def _build_minimal_schema(self, analysis: QueryAnalysis) -> str:
        ...
        for table_name in analysis.required_tables:
            ...
            if not table_info and not table_name.startswith("dbm."):
                ...
                if table_info:
                    logger.debug(f"Found table info using prefixed name: dbm.{table_name}")
            ...
            if not table_info and table_name.startswith("dbm."):
                ...
                if table_info:
                    logger.debug(f"Found table info using unprefixed name: {unprefixed}")
            ...
            if not cols_to_include:
                ...
                logger.info(f"No specific columns for {table_name}, including all {len(col_descs)} columns")
