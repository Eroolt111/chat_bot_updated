...
**TASKS:**
1. Check RELEVANT EXAMPLES first - if match found, use same table(s), columns, and logic. Examples are proven correct.
...
2. SQL GENERATOR **MUST RE-CALCULATE** the entity ID using a CTE based on the previous logic. So
   **EXTRACT ALL COLUMNS from [Previous SQL]** - not just the new ones needed!
   - Look at the WHERE clause filters (e.g., `class_name = 'ХЭВИЙН'`)
   - Look at the ORDER BY clause (e.g., `adv_amount * rate`)
   - Look at the SELECT columns
3. Include ALL those columns in `required_columns` because we must rebuild the CTE
4. Example: If previous SQL had `WHERE class_name = 'ХЭВИЙН' ORDER BY (adv_amount * rate) DESC`:
   - Include: `class_name`, `adv_amount`, `rate` (for CTE recreation)
   - PLUS: columns needed for current question (e.g., `status`)
