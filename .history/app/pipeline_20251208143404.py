...
class TableSelection(BaseModel):
    """A single table selected for the query."""
    table_name: str = Field(..., description="Full table name (e.g., DBM.LOAN_BALANCE_DETAIL)")
    fields: List[str] = Field(..., description="List of field names needed from this table")
    reason: str = Field(..., description="Why this table is needed")


class SchemaAnalysisResult(BaseModel):
    """Result of schema analysis (table/field selection)."""
    in_scope: bool = Field(..., description="Whether the query can be answered with available tables")
    out_of_scope_reason: str = Field(..., description="Reason if in_scope is false, empty string otherwise")
    relevant_tables_json: str = Field(..., description="JSON array of table selections. Example: '[{\"table_name\":\"DBM.LOAN\",\"fields\":[\"col1\"],\"reason\":\"needed\"}]'")
    relationships_json: str = Field(..., description="JSON array of join relationships. Example: '[\"table1.id = table2.id\"]'")
    needs_deduplication: bool = Field(..., description="Whether query needs deduplication logic")
    needs_chat_history: bool = Field(..., description="Whether query depends on previous conversation")
    chat_history_reasoning: str = Field(..., description="How previous context is used, empty if not needed")
    
    def get_relevant_tables(self) -> List[Dict]:
        """Parse relevant_tables_json to list."""
        try:
            return json.loads(self.relevant_tables_json)
        except:
            return []
    
    def get_relationships(self) -> List[str]:
        """Parse relationships_json to list."""
        try:
...
# --- PII Masking Functions ---
    ...
    def _triage_query(self, query: str) -> TriageResult:
        ...
        data_keywords = ["зээл", "данс", "үлдэгдэл", "төлбөр", "харилцагч", "компани", 
                         "хамгийн", "дүн", "хэмжээ", "мэдээлэл", "хуваарь", "олгосон",
    ...
    def _analyze_schema(self, query: str, context_str: str, history_summary: HistorySummary, 
                        retrieved_examples: List[Dict]) -> SchemaAnalysisResult:
        """
        NEW: Analyze which tables and fields are needed.
        This REPLACES table/column selection from the old analyzer.
        """
        logger.info("--- [SCHEMA ANALYSIS] Starting table/field selection...")
        ...
        history_context = ""
        if history_summary and history_summary.summary != "No previous conversation":
            history_context = f"""
Previous conversation summary: {history_summary.summary}
Key entities mentioned: {history_summary.get_key_entities()}
Last SQL pattern: {history_summary.last_sql_pattern}
"""
        
        try:
            schema_prompt_tmpl = ChatPromptTemplate(
                message_templates=[ChatMessage.from_str(
                    prompt_manager.get_schema_analysis_prompt().template,
                    role="user"
                )]
            )
            aux_llm = llm_manager.get_aux_llm()
            ...
            logger.info(f"--- [SCHEMA ANALYSIS] In scope: {result.in_scope}")
            if result.in_scope:
                tables = result.get_relevant_tables()
                logger.info(f"--- [SCHEMA ANALYSIS] Selected tables: {[t.get('table_name') for t in tables]}")
            else:
                logger.info(f"--- [SCHEMA ANALYSIS] Out of scope: {result.out_of_scope_reason}")
            
            self._stage_outputs['schema_analysis'] = result
            return result
            
        except Exception as e:
            logger.error(f"--- [SCHEMA ANALYSIS] Failed: {e}")
            # Fallback: allow query to proceed with full context
    ...
    def _validate_answer(self, query: str, sql_query: str, results: List[Dict]) -> AnswerValidation:
        """
        NEW: Post-execution validation - check if results actually answer the question.
        """
        ...
        if results and isinstance(results, list):
            ...
            if results and isinstance(results[0], dict) and "error" in results[0]:
                logger.info("--- [VALIDATION] Results contain error -> NOT ANSWERED")
                return AnswerValidation(
                    is_answered=False,
                    reason=f"SQL execution error: {results[0].get('error', 'Unknown error')}",
                    suggestion="Fix the SQL syntax or logic"
                )
            
            # If we have data, it's likely answered
            if len(results) > 0 and not any("error" in str(r).lower() for r in results):
                logger.info("--- [VALIDATION] Results contain data -> ANSWERED (quick check)")
                return AnswerValidation(
                    is_answered=True,
                    reason="Results contain relevant data",
                    suggestion=""
                )
        
        # Empty results might be valid (e.g., "no loans found")
        # Use LLM to determine if empty is acceptable
        try:
            validate_prompt_tmpl = ChatPromptTemplate(
                message_templates=[ChatMessage.from_str(
                    prompt_manager.get_validate_answer_prompt().template,
                    role="user"
                )]
            )
            aux_llm = llm_manager.get_aux_llm()
            ...
            result = aux_llm.structured_predict(
                AnswerValidation,
                validate_prompt_tmpl,
                query_str=query,
                sql_query=sql_query,
                results=str(results)[:2000]  # Truncate for token limits
            )
            
            logger.info(f"--- [VALIDATION] Result: {'ANSWERED' if result.is_answered else 'NOT ANSWERED'}")
            if not result.is_answered:
                logger.info(f"--- [VALIDATION] Reason: {result.reason}")
            
            self._stage_outputs['validation'] = result
            return result
            
        except Exception as e:
            logger.error(f"--- [VALIDATION] Failed: {e}. Assuming answer is valid.")
            return AnswerValidation(
                is_answered=True,
    ...
    def _build_minimal_schema(self, analysis: QueryAnalysis) -> str:
        ...
        for table_name in analysis.required_tables:
            ...
            cols_to_include = analysis.get_required_columns().get(table_name, [])
    ...
    def _execute_pipeline(self, enriched_query: str, original_user_query: str) -> str:
        ...
        This replaces the deprecated QueryPipeline.run() with direct method calls.
        The logic is identical to the original pipeline, just executed sequentially.
        """
        ...
        # ========== STEP 10: Review and correct SQL ==========
        reviewed_sql = self._review_and_correct_sql(
            sql_result=sql_result,
            context_str=context_str,
            masked_query_str=masked_query,
            analysis=analysis,
            retrieved_examples=retrieved_examples
        )
        
        # ========== STEP 11: Lint SQL ==========
        final_sql = self._lint_and_fix_sql(reviewed_sql)
    ...
    def run_query(self, user_query: str, memory: Optional[Any] = None, session_id: str = "default") -> str:
        ...
        try:
            ...
            # Schema analysis (new)
            schema_analysis = stage.get("schema_analysis")
            if schema_analysis:
                tables = schema_analysis.get_relevant_tables()
                selected_tables = [t.get("table_name") for t in tables] if tables else selected_tables
            
            # Query analyzer (legacy, still used)
